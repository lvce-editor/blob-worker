...
export const remove = async (dirent: string): Promise<void> => {
  await ExtensionHost.invoke('blobMemory.remove', dirent)
  // Trigger file watchers for memfs files
  await FileWatcher.triggerMemfsFileWatcher(dirent)
}

export const readFile = async (uri: string): Promise<string> => {
  return ExtensionHost.invoke('blobMemory.readFile', uri)
}
...
export const rename = async (oldUri: string, newUri: string): Promise<void> => {
  await ExtensionHost.invoke('blobMemory.rename', oldUri, newUri)
...
export class blobMemory implements blobHandler {}
