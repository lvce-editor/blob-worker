...
import * as blobFileHandle from '../src/parts/blobFileHandle/blobFileHandle.ts'
...
test('getFile', async () => {
  ...
  const result = await blobFileHandle.getFile(mockHandle)
...
test('getBinaryString', async () => {
  ...
  const result = await blobFileHandle.getBinaryString(mockHandle)
...
test('write', async () => {
  ...
  const mockWritable = {
    close: mockClose,
    write: mockWrite,
  } as unknown as blobWritableFileStream
  const mockCreateWritable = jest.fn<() => Promise<blobWritableFileStream>>().mockResolvedValue(mockWritable)
  const mockHandle = {
    createWritable: mockCreateWritable,
  } as unknown as blobFileHandle
  await blobFileHandle.write(mockHandle, 'content')
...
test('writeResponse', async () => {
  ...
  await blobFileHandle.writeResponse(mockHandle, mockResponse)
  expect(mockCreateWritable).toHaveBeenCalled()
  expect(mockBodyPipeTo).toHaveBeenCalledWith(mockWritable)
})

test('writeResponse with null body', async () => {
  const mockWritable = {} as unknown as blobWritableFileStream
  const mockCreateWritable = jest.fn<() => Promise<blobWritableFileStream>>().mockResolvedValue(mockWritable)
  const mockHandle = {
    createWritable: mockCreateWritable,
  } as unknown as blobFileHandle
