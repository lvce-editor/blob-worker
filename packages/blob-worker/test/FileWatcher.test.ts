...
test('watchFile should register watch callback and invoke blobProcess', async () => {
  mockInvoke.mockImplementation(async (method: string) => {
    if (method === 'blob.watchFile') {
      return
    }
...
test('unwatchFile should unregister watch callback and invoke blobProcess', async () => {
  mockInvoke.mockImplementation(async (method: string) => {
    if (method === 'blob.unwatchFile') {
      return
    }
    throw new Error(`unexpected method ${method}`)
  })
...
test('watchFile should register memfs watcher without invoking blobProcess', async () => {
  ...
  // Should not call blobProcess for memfs URIs
